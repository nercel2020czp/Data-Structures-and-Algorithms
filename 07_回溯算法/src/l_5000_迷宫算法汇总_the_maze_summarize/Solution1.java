...
//	        p = Integer.parseInt(str[2]);
		...
		new Solution1().backtrack(0, 0, array);
...
//		        做选择
//		        backtrack(路径, 选择列表)
	private void backtrack(int i, int j, int[][] array) {
		...
		// 向右走
		if (canMove(i, j, i, j + 1, array)) {//排除不合法选择
			// ------------做出选择--------------
		...
		// 向下走
		if (canMove(i, j, i + 1, j, array)) {//排除不合法选择
			// ------------做出选择--------------
			path.addLast("下");//保存路径位置
			// 标记为已走,已走过的点置标志位5
			array[i][j] = 5;
			// 从下一个点继续寻路,递归调用
			backtrack(i + 1, j, array);
			// ------------撤销选择--------------
			// 均不可行,则恢复现场
			array[i][j] = 0;//这一步非常重要,进行回溯
			path.removeLast();
	 ...
	 * @return boolean 可否通过
	 */
	private boolean canMove(int i, int j, int targetI, int targetJ, int[][] array) {
